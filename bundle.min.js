...
"use strict";Mob.prototype=Object.create(Entity.prototype);function Mob(x,y,depth,config,actions,animations){Entity.call(this,x,y,depth,config.size,display_opaque,physical_solid,animations);this.components.push(component_direction,component_combat,component_actions,component_collision);this.health=new HealthComponent(config.health);this.direction=new DirectionComponent(direction_down);this.combat=new CombatComponent(initialStrength,initialDexterity,initialIntelligence);this.actions=new ActionsComponent(actions);if(config.solid){this.components.push(component_collision);this.collision=new CollisionComponent(x,y,config.size,config.size)}if(config.sprint_threshhold!==undefined){this.components.push(component_sprint);this.sprint=new SprintComponent(config.sprint_threshhold,config.sprint_speed)}this.components.push(component_ai);this.ai=new AIComponent(config.attack_range)}
"use strict";Player.prototype=Object.create(Entity.prototype);function Player(x,y,config,playerClass,actions,animations){Entity.call(this,x,y,0,1,display_opaque,physical_solid,animations);this.components.push(component_direction,component_health,component_combat,component_actions,component_collision,component_sprint,component_light_emitter);this.direction=new DirectionComponent(direction_down);this.health=new HealthComponent(config.health);this.combat=new CombatComponent(config.initialStrength,config.initialDexterity,config.initialIntelligence);this.actions=new ActionsComponent(actions);this.sprint=new SprintComponent(config.sprint_threshhold,config.sprint_speed);this.lightEmitter=new LightEmitterComponent(light_level_player)}
