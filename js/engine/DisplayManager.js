...
function DisplayManager(square_size, vision, animation_stages) {
	...
	let drawDungeonDebug = function (board, player) {
		...
		let XOFF = floor((width - (SQUARE_SIZE * CONFIG.DUNGEON_SIZE))/2);
		...
		for (let x = 0; x < CONFIG.DUNGEON_SIZE; x++) {
			for (let y = 0; y < CONFIG.DUNGEON_SIZE; y++) {
				...
				rect(XOFF + (x * SQUARE_SIZE), 100 + (y * SQUARE_SIZE), SQUARE_SIZE, SQUARE_SIZE);
			}
		}
		fill(255, 255, 0);
		rect(XOFF + (player.x * SQUARE_SIZE) + PLAYER_OFFSET, 100 + (player.y * SQUARE_SIZE) + PLAYER_OFFSET, PLAYER_SIZE, PLAYER_SIZE);
	}
